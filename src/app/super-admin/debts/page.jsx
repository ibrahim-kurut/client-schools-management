...
export default function DebtsManagement() {
  ...
  const handleSettleDebt = async (schoolId, schoolName, debtAmount) => {
    const result = await Swal.fire({
      title: 'تسوية الديون',
      text: `هل أنت متأكد من تسوية ديون "${schoolName}" البالغة $${debtAmount}؟ يجب التأكد من استلام المبلغ يدوياً أولاً.`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#10b981',
      cancelButtonColor: '#64748b',
      confirmButtonText: 'نعم، تمت التسوية',
    ...
    if (result.isConfirmed) {
      try {
        await axios.post(`/subscriptions/settle-debt/${schoolId}`);
        toast.success(`تمت تسوية ديون ${schoolName} بنجاح`);
                      ...
                      <td className="px-6 py-6">
                        {debt > 0 ? (
                          <button 
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-[11px] font-black transition-all shadow-lg shadow-emerald-500/20 active:scale-95"
                          >
                            <HandCoins className="w-3.5 h-3.5" />
                            تسوية الدين
                          </button>
                        ) : (
                          <div className="flex items-center gap-2 text-emerald-500 font-bold text-[10px] uppercase tracking-widest px-4 py-2 bg-emerald-50 dark:bg-emerald-500/10 rounded-xl border border-emerald-100 dark:border-emerald-500/20">
